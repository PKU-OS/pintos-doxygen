...
The result is stored in the [html](/html) folder. View it [here](https://PKU-OS.github.io/pintos-doxygen/html/index.html).
...
## Usage
Copy the [Doxyfile](Doxyfile) file to your pintos source root (the source root is not the src directory in your pintos source, but its parent directory). Then execute `doxygen` 
in the source root.
...
```bash
python2 convert_doxygen_comments.py /path/to/pintos/src/threads/init.c
```

Or convert the entire source tree:

```bash
python2 convert_doxygen_comments.py /path/to/pintos/src
```
...
```
python2 convert_doxygen_comments.py --in_place /path/to/pintos/src
```
